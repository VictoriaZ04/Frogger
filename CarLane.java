...
public class CarLane extends Lane {
    ...
    public void update() {
        super.update();
        if (getItems().size() == 0) {
            getItems().add(new Car((direction == 3) ? 800 : 0, y, direction, speed, (int) (Math.random() * 4)));
        }
        
        while (getItems().size() < 5) {
            getItems().add(new Car(getItems().get(getItems().size() - 1).getX() + (((getItems().get(getItems().size() - 1).getRectangle().getWidth()) + 170) * ((getDirection() == 2)?1:-1)), y, direction, speed, (int) (Math.random() * 4)));
